...
// Iluminação por vértice para a vaca
in vec3 cowIllumination;
...
void main()
{
    ...
    vec3 Ka = vec3(0.3, 0.3, 0.3); // Refletância ambiente
    ...
    if (object_id == BUNNY) {
        color = Kd * (lambert + 0.01) + ambient_term + phong_specular_term; // Adiciona blinn-phong ao modelo de lambert
    } else if (object_id == COW) {
        color = cowIllumination;
    } else {
        color = Kd * (lambert + 0.01 + ambient_term);
    }
